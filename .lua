...
local OrionLib = {
	...
	Themes = {
		Default = {
			-- ÄNDERUNG: Dunklerer Hintergrund (Main + Second dunkler)
			Main = Color3.fromRGB(15, 15, 15),
			Second = Color3.fromRGB(20, 20, 20),
			-- ÄNDERUNG: Stroke/Divider leicht rot
...
-- Accent-Farbe (leicht rot) für Toggles, Slider, etc.
local AccentRed = Color3.fromRGB(180, 50, 50)
...
function OrionLib:MakeNotification(NotificationConfig)
	spawn(function()
		...
		local NotificationFrame = SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(15, 15, 15), 0, 10), {
			...
			-- ÄNDERUNG: Roter Stroke für Notifications
			MakeElement("Stroke", Color3.fromRGB(120, 40, 40), 1.2),
...
function OrionLib:MakeWindow(WindowConfig)
	...
	-- ÄNDERUNG: Roter ScrollBar für Tab-Holder
	local TabHolder = AddThemeObject(SetChildren(SetProps(MakeElement("ScrollFrame", Color3.fromRGB(180, 50, 50), 4), {
	...
	-- ÄNDERUNG: Such-Button (Lupe) links im Button-Block
	local SearchBtn = SetChildren(SetProps(MakeElement("Button"), {
		Size = UDim2.new(0.333, 0, 1, 0),
		Position = UDim2.new(0, 0, 0, 0),
		BackgroundTransparency = 1
	}), {
		AddThemeObject(SetProps(MakeElement("Image", "rbxassetid://3926305904"), { -- Lupe Icon
			Position = UDim2.new(0, 9, 0, 6),
			Size = UDim2.new(0, 18, 0, 18),
			Name = "Ico"
		}), "Text")
	...
	local WindowStuff = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 10), {
		...
		-- ÄNDERUNG: Roter Divider-Strich rechts der Seitenleiste
	...
	-- ÄNDERUNG: Roter TopBar-Trennstrich
	...
	-- ÄNDERUNG: Suchleiste (SearchBar) — standardmäßig unsichtbar
	local SearchBar = Create("TextBox", {
		Size = UDim2.new(1, -20, 0, 26),
		Position = UDim2.new(0, 10, 0, 12),
		BackgroundColor3 = Color3.fromRGB(30, 10, 10),
		TextColor3 = Color3.fromRGB(240, 240, 240),
		PlaceholderColor3 = Color3.fromRGB(120, 80, 80),
		...
		Create("UIStroke", {Color = Color3.fromRGB(150, 50, 50), Thickness = 1})
	})
	...
	local MainWindow = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 10), {
		...
		SetChildren(SetProps(MakeElement("TFrame"), {
			...
			WindowName,
			WindowTopBarLine,
			-- ÄNDERUNG: Dreier-Button-Block (Lupe | Minimize | Close) mit rotem Rahmen
			AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 7), {
				Size = UDim2.new(0, 100, 0, 30),
				Position = UDim2.new(1, -120, 0, 10)
			}), {
				-- ÄNDERUNG: Roter Stroke um den Button-Block
				SetProps(MakeElement("Stroke"), {
					Color = Color3.fromRGB(120, 40, 40),
					Thickness = 1
				}),
				-- Trennlinie Lupe | Minimize
				AddThemeObject(SetProps(MakeElement("Frame"), {
					Size = UDim2.new(0, 1, 1, 0),
					Position = UDim2.new(0.333, 0, 0, 0)
				}), "Stroke"),
				-- Trennlinie Minimize | Close
				AddThemeObject(SetProps(MakeElement("Frame"), {
					Size = UDim2.new(0, 1, 1, 0),
					Position = UDim2.new(0.666, 0, 0, 0)
				}), "Stroke"),
	...
	-- ÄNDERUNG: Such-Button Logik — öffnet/schließt SearchBar und filtert Tabs
	AddConnection(SearchBtn.MouseButton1Up, function()
		...
		if SearchOpen then
			...
			SearchBar.Text = ""
			-- Alle Tab-Einträge wieder zeigen
	...
	-- Live-Suche: blendet Tabs aus, die nicht zum Suchbegriff passen
		...
		for _, Tab in next, TabHolder:GetChildren() do
			if Tab:IsA("TextButton") then
				if Query == "" then
					...
					-- Tab hat ein Title-Label (auch wenn unsichtbar) — wir suchen darin
					local TitleLabel = Tab:FindFirstChild("Title")
					if TitleLabel then
						Tab.Visible = TitleLabel.Text:lower():find(Query, 1, true) ~= nil
					end
				end
			end
		end
	...
	function TabFunction:MakeTab(TabConfig)
		...
		-- ÄNDERUNG: Tab-Frame nur mit Icon, kein sichtbarer Name
		-- Breite reduziert auf 40px (nur Icon), zentriert
		local TabFrame = SetChildren(SetProps(MakeElement("Button"), {
			...
			-- Title-Label bleibt für die Suchfunktion erhalten, aber unsichtbar
			SetProps(MakeElement("Label", TabConfig.Name, 14), {
				Size = UDim2.new(0, 0, 0, 0),
				Visible = false,
				Name = "Title"
			}),
			-- ÄNDERUNG: Roter linker Indikator-Strich für aktiven Tab
			SetProps(MakeElement("Frame"), {
				Size = UDim2.new(0, 3, 0, 20),
				Position = UDim2.new(0, 0, 0.5, -10),
				BackgroundColor3 = AccentRed,
				BorderSizePixel = 0,
				Visible = false,
				Name = "ActiveBar"
		...
		local Container = AddThemeObject(SetChildren(SetProps(MakeElement("ScrollFrame", Color3.fromRGB(180, 50, 50), 5), {
		...
		if FirstTab then
			...
			TabFrame.ActiveBar.Visible = true
		...
		AddConnection(TabFrame.MouseButton1Click, function()
			for _, Tab in next, TabHolder:GetChildren() do
				if Tab:IsA("TextButton") then
					TweenService:Create(Tab.Ico, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {ImageTransparency = 0.5}):Play()
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddLabel(Text)
				local LabelFrame = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 5), {
					...
					-- ÄNDERUNG: Roter Stroke
					SetProps(MakeElement("Stroke"), {Color = AccentRed, Thickness = 1})
				}), "Second")
			...
			function ElementFunction:AddToggle(ToggleConfig)
				...
				-- ÄNDERUNG: Standard-Toggle-Farbe rot
				ToggleConfig.Color = ToggleConfig.Color or AccentRed
			...
			function ElementFunction:AddSlider(SliderConfig)
				...
				-- ÄNDERUNG: Standard-Slider-Farbe rot
				SliderConfig.Color = SliderConfig.Color or AccentRed
			...
			function ElementFunction:AddBind(BindConfig)
				...
				local BindBox = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 4), {
					...
					-- ÄNDERUNG: Roter Stroke für Bind-Box
					SetProps(MakeElement("Stroke"), {Color = AccentRed, Thickness = 1}),
			...
			function ElementFunction:AddTextbox(TextboxConfig)
				...
				local TextContainer = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 4), {
					...
					-- ÄNDERUNG: Roter Stroke für Textbox
					SetProps(MakeElement("Stroke"), {Color = AccentRed, Thickness = 1}),
					TextboxActual
				}), "Main")
