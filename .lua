--V2 Venty Library – Category + Tab System (Vanity-Style)
...
local function LoadCfg(Config)
    ...
    table.foreach(Data, function(a,b)
        if Library.Flags[a] then
            spawn(function()
                if Library.Flags[a].Type == "Colorpicker" then Library.Flags[a]:Set(UnpackColor(b))
                else Library.Flags[a]:Set(b) end
            end)
        end
    end)
end
...
CreateElement("Image",       function(ImageID)         return Create("ImageLabel",  {Image=ImageID, BackgroundTransparency=1}) end)
...
CreateElement("Label",       function(Text, TextSize, Transparency)
    return Create("TextLabel",{Text=Text or "",TextColor3=Color3.fromRGB(240,240,240),TextTransparency=Transparency or 0,TextSize=TextSize or 15,Font=Enum.Font.GothamSemibold,RichText=true,BackgroundTransparency=1,TextXAlignment=Enum.TextXAlignment.Left})
end)
    ...
    if Library.SaveCfg then
        pcall(function()
            if isfile(Library.Folder.."/"..game.GameId..".txt") then
                ...
                Library:MakeNotification({Name="Configuration",Content="Auto-loaded configuration for "..game.GameId..".",Time=5})
            end
        end)
    end
end
...
local function ShowLoadingScreen(duration, callback)
    local Overlay = Create("Frame",{Parent=Container,Size=UDim2.new(1,0,1,0),BackgroundColor3=Color3.fromRGB(0,0,0),BackgroundTransparency=1,ZIndex=100,BorderSizePixel=0})
    ...
    for i, stepText in ipairs(steps) do
        ...
        while tick()-t0 < stepDur do
            local p = math.clamp((tick()-t0)/stepDur,0,1)
            PercLabel.Text = math.floor(startPct+(endPct-startPct)*p).."%"
            task.wait()
        end
...
function Library:MakeWindow(WindowConfig)
    ...
    local WindowName = AddThemeObject(SetProps(MakeElement("Label",WindowConfig.Name,16),{
        ...
        Font=Enum.Font.GothamBlack, TextSize=16, TextXAlignment=Enum.TextXAlignment.Center
    }),"Text")
    ...
    if WindowConfig.BannerIcon then
        BannerIconLabel = SetProps(MakeElement("Image",WindowConfig.BannerIcon),{
            AnchorPoint=Vector2.new(0,0.5), Position=UDim2.new(0,12,0.5,0), Size=UDim2.new(0,26,0,26), Name="BannerIcon"
        })
    end

    local CloseBtn = SetChildren(SetProps(MakeElement("Button"),{Size=UDim2.new(0,32,1,0),Position=UDim2.new(0,76,0,0),BackgroundTransparency=1}),{
        AddThemeObject(SetProps(MakeElement("Image","rbxassetid://7072725342"),{AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(0.5,0,0.5,0),Size=UDim2.new(0,16,0,16)}),"Text")
    })
    local MinimizeBtn = SetChildren(SetProps(MakeElement("Button"),{Size=UDim2.new(0,32,1,0),Position=UDim2.new(0,38,0,0),BackgroundTransparency=1}),{
        AddThemeObject(SetProps(MakeElement("Image","rbxassetid://7072719338"),{AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(0.5,0,0.5,0),Size=UDim2.new(0,16,0,16),Name="Ico"}),"Text")
    })
    local ResizeBtn = SetChildren(SetProps(MakeElement("Button"),{Size=UDim2.new(0,32,1,0),Position=UDim2.new(0,0,0,0),BackgroundTransparency=1}),{
        AddThemeObject(SetProps(MakeElement("Image","rbxassetid://117273761878755"),{AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(0.5,0,0.5,0),Size=UDim2.new(0,16,0,16)}),"Text")
    })

    -- Search button (Lupe) – 4. Icon in der Button-Gruppe
    local SearchBtn = SetChildren(SetProps(MakeElement("Button"),{Size=UDim2.new(0,32,1,0),Position=UDim2.new(0,0,0,0),BackgroundTransparency=1}),{
        AddThemeObject(SetProps(MakeElement("Image","rbxassetid://6031094678"),{AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(0.5,0,0.5,0),Size=UDim2.new(0,15,0,15),Name="Ico"}),"Text")
    })

    -- ButtonContainer: 4 Buttons × 32px = 128px + 4px Rand = 144px
    local ButtonContainer = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame",Color3.fromRGB(255,255,255),0,6),{
        Size=UDim2.new(0,140,0,26), Position=UDim2.new(1,-8,0.5,0), AnchorPoint=Vector2.new(1,0.5)
    }),{
        AddThemeObject(MakeElement("Stroke"),"Stroke"),
        -- Divider lines between buttons
        AddThemeObject(SetProps(MakeElement("Frame"),{Size=UDim2.new(0,1,1,0),Position=UDim2.new(0,35,0,0)}),"Stroke"),
        AddThemeObject(SetProps(MakeElement("Frame"),{Size=UDim2.new(0,1,1,0),Position=UDim2.new(0,70,0,0)}),"Stroke"),
        AddThemeObject(SetProps(MakeElement("Frame"),{Size=UDim2.new(0,1,1,0),Position=UDim2.new(0,105,0,0)}),"Stroke"),
        -- Buttons left→right: Search, Resize, Minimize, Close
        SetProps(SearchBtn,   {Position=UDim2.new(0,2,0,0),   Size=UDim2.new(0,33,1,0)}),
        SetProps(ResizeBtn,   {Position=UDim2.new(0,37,0,0),  Size=UDim2.new(0,33,1,0)}),
        SetProps(MinimizeBtn, {Position=UDim2.new(0,72,0,0),  Size=UDim2.new(0,33,1,0)}),
        SetProps(CloseBtn,    {Position=UDim2.new(0,107,0,0), Size=UDim2.new(0,33,1,0)})
    }),"Second")

    local DragPoint = SetProps(MakeElement("TFrame"),{Size=UDim2.new(1,0,0,46)})

    -- ── Category Screen (Startseite – Vanity-Stil) ────────────────────────────

    -- Diese Seite zeigt alle Kategorien als große Kacheln
    local CategoryScreen = AddThemeObject(SetProps(MakeElement("TFrame"),{
    ...
    local CategoryGrid = SetProps(Create("UIGridLayout",{
        CellSize        = UDim2.new(0,165,0,90),
        CellPadding     = UDim2.new(0,10,0,10),
        SortOrder       = Enum.SortOrder.LayoutOrder,
        HorizontalAlignment = Enum.HorizontalAlignment.Center,
        VerticalAlignment   = Enum.VerticalAlignment.Center
    }),{Parent=CategoryScreen})

    -- ── Tab Screen (nach Klick auf Kategorie) ─────────────────────────────────

    local TabScreen = AddThemeObject(SetProps(MakeElement("TFrame"),{
        Size    = UDim2.new(1,0,1,-46),
        Position= UDim2.new(0,0,0,46),
        Visible = false,
        Name    = "TabScreen"
    ...
    -- Leiste oben im TabScreen: [← Back]  [Kategoriename]
    local TabScreenHeader = AddThemeObject(SetChildren(SetProps(MakeElement("Frame"),{
        Size=UDim2.new(1,0,0,TAB_HEIGHT),
        Name="TabScreenHeader"
    }),{
        -- Back-Button (Pfeil + Text)
        SetChildren(SetProps(MakeElement("Button"),{
            ...
            AddThemeObject(SetProps(MakeElement("Image","rbxassetid://7072344541"),{
                AnchorPoint=Vector2.new(0,0.5),
                Position=UDim2.new(0,8,0.5,0),
                Size=UDim2.new(0,14,0,14)
            }),"TextDark"),
            AddThemeObject(SetProps(MakeElement("Label","Back",13),{
                AnchorPoint=Vector2.new(0,0.5),
        ...
        -- Divider | rechts vom Back-Button
        AddThemeObject(SetProps(MakeElement("Frame"),{
            Size=UDim2.new(0,1,0.6,0),
            AnchorPoint=Vector2.new(0,0.5),
            Position=UDim2.new(0,80,0.5,0)
        }),"Stroke"),
        -- Aktueller Kategoriename
        AddThemeObject(SetProps(MakeElement("Label","",13),{
            AnchorPoint=Vector2.new(0,0.5),
            Position=UDim2.new(0,94,0.5,0),
    ...
    TabScreenHeader.Parent = TabScreen

    -- Tab-Bar (horizontale Tabs innerhalb der Kategorie)
    local TabBar = AddThemeObject(SetChildren(SetProps(MakeElement("ScrollFrame",Color3.fromRGB(255,255,255),0),{
        Size=UDim2.new(1,0,0,TAB_HEIGHT),
        Position=UDim2.new(0,0,0,TAB_HEIGHT),
    ...
    local TabBarLine = AddThemeObject(SetProps(MakeElement("Frame"),{
        Size=UDim2.new(1,0,0,1),
        Position=UDim2.new(0,0,0,TAB_HEIGHT*2)
    }),"Stroke")
    TabBarLine.Parent = TabScreen

    -- Suchleiste (ausklappbar, erscheint unter TabBarLine)
    local SearchBarVisible = false
    local SearchBar = AddThemeObject(SetChildren(SetProps(MakeElement("Frame"),{
        Size=UDim2.new(1,0,0,0),
        Position=UDim2.new(0,0,0,TAB_HEIGHT*2+1),
        ClipsDescendants=true,
        Name="SearchBar"
    }),{
        MakeElement("Padding",5,10,10,5),
        ...
        Create("TextBox",{
            ...
            BackgroundTransparency=1,
            PlaceholderText="Funktion suchen...",
    ...
    SearchBar.Parent = TabScreen

    local ContentArea = AddThemeObject(SetProps(MakeElement("TFrame"),{
        Size=UDim2.new(1,0,1,-(TAB_HEIGHT*2+1)),
        Position=UDim2.new(0,0,0,TAB_HEIGHT*2+1),
        Name="ContentArea"
    }),"Main")
    ContentArea.Parent = TabScreen

    -- ── Search-Logik ──────────────────────────────────────────────────────────

    local function UpdateSearchLayout()
        local offset = SearchBarVisible and (TAB_HEIGHT*2+1+30) or (TAB_HEIGHT*2+1)
        SearchBar.Position = UDim2.new(0,0,0,TAB_HEIGHT*2+1)
        ContentArea.Position = UDim2.new(0,0,0,offset)
        ContentArea.Size     = UDim2.new(1,0,1,-offset)
    end

    AddConnection(SearchBtn.MouseButton1Up, function()
        SearchBarVisible = not SearchBarVisible
        if SearchBarVisible then
            TweenService:Create(SearchBar,TweenInfo.new(0.2,Enum.EasingStyle.Quint),{Size=UDim2.new(1,0,0,30)}):Play()
            task.wait(0.05)
            SearchBar.Input:CaptureFocus()
        else
            TweenService:Create(SearchBar,TweenInfo.new(0.2,Enum.EasingStyle.Quint),{Size=UDim2.new(1,0,0,0)}):Play()
    ...
    AddConnection(SearchBar.Input:GetPropertyChangedSignal("Text"), function()
        ...
        for _, Cont in pairs(ContentArea:GetChildren()) do
            if Cont.Name == "ItemContainer" then
                Cont.Visible = true
                for _, El in pairs(Cont:GetChildren()) do
                    -- Section-Holder
                    if El:IsA("Frame") and El:FindFirstChild("Holder") then
                        for _, Item in pairs(El.Holder:GetChildren()) do
                            if Item:IsA("Frame") and Item:FindFirstChild("Content") then
                                Item.Visible = query == "" or string.find(string.lower(Item.Content.Text or ""),query,1,true) ~= nil
                            end
                        end
                    end
                    -- Direkte Items
                    if El:IsA("Frame") and El:FindFirstChild("Content") and not El:FindFirstChild("Holder") then
                        El.Visible = query == "" or string.find(string.lower(El.Content.Text or ""),query,1,true) ~= nil
                    end
                end
            end
        end
    ...
    local TopBarChildren = {WindowName, WindowTopBarLine, ButtonContainer}
    if BannerIconLabel then table.insert(TopBarChildren, BannerIconLabel) end
    ...
    local MainWindow = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame",Color3.fromRGB(255,255,255),0,10),{
        ...
        SetChildren(SetProps(MakeElement("TFrame"),{Size=UDim2.new(1,0,0,46),Name="TopBar",ClipsDescendants=false}),TopBarChildren),
        DragPoint,
        CategoryScreen,
        TabScreen
    }),"Main")
    ...
    local function ShowCategoryScreen()
        -- Suche zurücksetzen
        if SearchBarVisible then
        ...
        -- Alle TabBar-Buttons & ItemContainer leeren (wird neu befüllt pro Kategorie)
        -- Screens wechseln
        TweenService:Create(TabScreen,TweenInfo.new(0.22,Enum.EasingStyle.Quint),{Position=UDim2.new(1,0,0,46)}):Play()
        task.wait(0.22)
        TabScreen.Visible      = false
        TabScreen.Position     = UDim2.new(0,0,0,46)
        CategoryScreen.Visible = true
        TweenService:Create(CategoryScreen,TweenInfo.new(0.22,Enum.EasingStyle.Quint),{Position=UDim2.new(0,0,0,46)}):Play()
    end

    local function ShowTabScreen(categoryName)
        TabScreenHeader.CategoryLabel.Text = categoryName
        CategoryScreen.Visible = false
        TabScreen.Position     = UDim2.new(1,0,0,46)
        TabScreen.Visible      = true
        TweenService:Create(TabScreen,TweenInfo.new(0.22,Enum.EasingStyle.Quint),{Position=UDim2.new(0,0,0,46)}):Play()
    end

    AddConnection(TabScreenHeader.BackBtn.MouseButton1Up, function()
        ShowCategoryScreen()
    end)
    ...
    local MobileReopenButton = SetChildren(SetProps(MakeElement("Button"),{
        Parent=Container, Size=UDim2.new(0,40,0,40), Position=UDim2.new(0.5,-20,0,20),
        BackgroundColor3=Library.Themes[Library.SelectedTheme].Main, Visible=false
    }),{
    ...
    AddConnection(MinimizeBtn.MouseButton1Up, function()
        if Minimized then
            ...
            CategoryScreen.Visible = not TabScreen.Visible
            TabScreen.Visible      = TabScreen.Visible
    ...
    -- ── MakeCategory ──────────────────────────────────────────────────────────
    -- Gibt MakeTab zurück, wie früher direkt verfügbar war.

    local CategoryFunction = {}

    function CategoryFunction:MakeCategory(CategoryConfig)
        CategoryConfig        = CategoryConfig or {}
        CategoryConfig.Name   = CategoryConfig.Name   or "Category"
        CategoryConfig.Icon   = CategoryConfig.Icon   or "rbxassetid://0"
        CategoryConfig.Color  = CategoryConfig.Color  or Color3.fromRGB(60,120,255)

        local FirstTab   = true
        -- Tabs für diese Kategorie
        local CategoryTabBar     = {}  -- wir nutzen den globalen TabBar
        local CategoryContainers = {}  -- ItemContainer dieser Kategorie

        -- ── Kategorie-Kachel ────────────────────────────────────────────────

        local TileClick = SetProps(MakeElement("Button"),{Size=UDim2.new(1,0,1,0),ZIndex=3})

        -- Farbiger Accent-Streifen oben
        local AccentBar = Create("Frame",{
            Size=UDim2.new(1,0,0,3),
            BackgroundColor3=CategoryConfig.Color,
            BorderSizePixel=0,
            ZIndex=2
        },{Create("UICorner",{CornerRadius=UDim.new(0,5)})})

        local TileIcon = SetProps(MakeElement("Image",CategoryConfig.Icon),{
            AnchorPoint=Vector2.new(0.5,0.5),
            Position=UDim2.new(0.5,0,0.45,0),
            Size=UDim2.new(0,28,0,28),
            ZIndex=2,
            ImageColor3=CategoryConfig.Color
        })
        local TileLabel = AddThemeObject(SetProps(MakeElement("Label",CategoryConfig.Name,14),{
        ...
        local CategoryTile = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame",Color3.fromRGB(255,255,255),0,8),{
            Parent=CategoryScreen,
        ...
        -- Hover-Effekte
        AddConnection(TileClick.MouseEnter,function()
            TweenService:Create(CategoryTile,TweenInfo.new(0.18,Enum.EasingStyle.Quint),{BackgroundColor3=Color3.fromRGB(22,22,26)}):Play()
        end)
        AddConnection(TileClick.MouseLeave,function()
            TweenService:Create(CategoryTile,TweenInfo.new(0.18,Enum.EasingStyle.Quint),{BackgroundColor3=Library.Themes[Library.SelectedTheme].Second}):Play()
        end)
        AddConnection(TileClick.MouseButton1Down,function()
            TweenService:Create(AccentBar,TweenInfo.new(0.1,Enum.EasingStyle.Quad),{Size=UDim2.new(1,0,0,5)}):Play()
        end)
        AddConnection(TileClick.MouseButton1Up,function()
            TweenService:Create(AccentBar,TweenInfo.new(0.15,Enum.EasingStyle.Quint),{Size=UDim2.new(1,0,0,3)}):Play()
        end)

        -- Klick → Tab-Ansicht öffnen
        AddConnection(TileClick.MouseButton1Click,function()
            -- Alle bisherigen TabBar-Kinder ausblenden (andere Kategorien)
            for _, tb in pairs(TabBar:GetChildren()) do
                if tb:IsA("TextButton") then tb.Visible = false end
            end
            for _, Cont in pairs(ContentArea:GetChildren()) do
                if Cont.Name == "ItemContainer" then Cont.Visible = false end
            end
            -- Tabs dieser Kategorie einblenden
            for _, tb in pairs(CategoryTabBar) do
                tb.Visible = true
            end
            -- ersten Tab aktivieren
            if FirstTab == false and #CategoryTabBar > 0 then
                -- ersten Tab dieser Kategorie aktivieren
                CategoryTabBar[1]:FireButton1Click()
            end
            ShowTabScreen(CategoryConfig.Name)
        end)

        -- ── MakeTab (innere Tabs der Kategorie) ─────────────────────────────
        ...
        function TabFunction:MakeTab(TabConfig)
            ...
            local TabBtn = AddThemeObject(SetChildren(SetProps(MakeElement("Button"),{
                ...
                Visible=false   -- zunächst unsichtbar (erst wenn Kategorie gewählt)
            }),{
                Create("Frame",{
                    Name="ActiveBar",
                ...
                Create("UIPadding",{PaddingRight=UDim.new(0,TAB_PADDING)})
            }),"Second")

            table.insert(CategoryTabBar, TabBtn)
            ...
            table.insert(CategoryContainers, ItemContainer)
            ...
            local function ActivateTab()
                -- Deactivate all tabs in this category
                for _, tb in pairs(CategoryTabBar) do
                    pcall(function()
                ...
                for _, Cont in pairs(CategoryContainers) do Cont.Visible = false end
                ...
                task.defer(function()
                    ...
                    elseif tabEnd > canvas+barW then
                        TabBar.CanvasPosition = Vector2.new(tabEnd-barW+8,0)
                    end
                end)
            end
            ...
            if FirstTab then
                FirstTab = false
                -- Beim Öffnen der Kategorie wird der erste Tab direkt aktiviert
                -- Wir speichern ActivateTab im Button für den Kategorie-Klick
                TabBtn:SetAttribute("IsFirst", true)
                task.defer(function()
                    -- ersten Tab sofort aktivieren wenn Kategorie geöffnet wird
                end)
            end

            -- Kategorie-Kachel: beim Klick ersten Tab aktivieren
            AddConnection(TileClick.MouseButton1Click, function()
                if TabBtn:GetAttribute("IsFirst") then
                    ActivateTab()
                end
            end)
            ...
            local function GetElements(ItemParent)
                ...
                function ElementFunction:AddToggle(ToggleConfig)
                    ...
                    },{Create("UICorner",{CornerRadius=UDim.new(1,0)}),Create("UIStroke",{Color=Color3.fromRGB(40,40,50),Thickness=1,Name="Stroke"})})
...
-- ── Kategorie "Visuals" ──────────────────────────────
local VisualsCategory = Window:MakeCategory({
    Name  = "Visuals",
    Icon  = "rbxassetid://ICON_ID_HIER",
    Color = Color3.fromRGB(60, 120, 255),
})

local ESPTab = VisualsCategory:MakeTab({ Name = "ESP" })
ESPTab:AddToggle({
    Name     = "Player ESP",
    Default  = false,
...
local ChamsTab = VisualsCategory:MakeTab({ Name = "Chams" })
ChamsTab:AddToggle({ Name = "Wall Chams", Default = false, Callback = function(v) end })

-- ── Kategorie "Main" ─────────────────────────────────
local MainCategory = Window:MakeCategory({
    Name  = "Main",
    Icon  = "rbxassetid://ICON_ID_HIER",
    Color = Color3.fromRGB(255, 80, 80),
})

local MoveTab = MainCategory:MakeTab({ Name = "Movement" })
MoveTab:AddToggle({ Name = "Speed",  Default = false, Callback = function(v) end })
MoveTab:AddToggle({ Name = "Fly",    Default = false, Callback = function(v) end })

-- ── Kategorie "Settings" ─────────────────────────────
local SettingsCategory = Window:MakeCategory({
    Name  = "Settings",
    Icon  = "rbxassetid://ICON_ID_HIER",
    Color = Color3.fromRGB(120, 120, 120),
})

local CfgTab = SettingsCategory:MakeTab({ Name = "Config" })
CfgTab:AddToggle({ Name = "Auto Save", Default = false, Callback = function(v) end })
